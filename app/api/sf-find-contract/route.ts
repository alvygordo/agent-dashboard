...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!oppResult.records.length) {
      return NextResponse.json({ error: 'Opportunity not found', contract: null })
    }
    ...
    return NextResponse.json({
      oppId:    opp.Id,
      oppName:  opp.Name,
      oppUrl:   `${instanceUrl}/lightning/r/Opportunity/${opp.Id}/view`,
      contract: scored[0] ?? null,
